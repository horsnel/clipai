  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        showBg
          ? 'bg-clip-dark/90 backdrop-blur-md border-b border-white/[0.08]'
          : 'bg-transparent'
      }`}
    >
      <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12">
        <div className="flex items-center justify-between h-20 lg:h-24">
              ...
              <button
                ...
                className={`group inline-flex items-center gap-1.5 px-2.5 xl:px-4 py-2 text-sm font-medium rounded-lg transition-all whitespace-nowrap ${
                  currentPage === link.page
                    ? 'text-clip-cyan bg-clip-cyan/10 border-l-2 border-clip-cyan'
                    : 'text-clip-muted hover:text-clip-text hover:bg-white/[0.06]'
                }`}
              >
                {link.label}
                {link.badge && (
                  <span
                    className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md border border-clip-cyan bg-clip-cyan/10 text-clip-cyan group-hover:bg-clip-cyan/15 transition-colors"
          ...
          <button
            ...
            className="lg:hidden p-2 text-clip-text hover:bg-white/[0.06] rounded-lg transition-colors"
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-clip-dark/98 backdrop-blur-lg border-b border-white/[0.08] max-h-[calc(100vh-4rem)] overflow-y-auto">
          <div className="px-4 py-4 space-y-2">
            {navLinks.map((link) => (
              ...
              >
                ...
                {link.badge && (
                  <span
                    className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md border border-clip-cyan bg-clip-cyan/10 text-clip-cyan"
                    title="This feature is under development"
                  >
                    {link.badge}
                  </span>
              ...
              {isLoggedIn ? null : (
                <div className="space-y-2">
                  <button
                    ...
                    className="w-full px-4 py-3 text-left text-sm font-medium text-clip-muted hover:text-clip-text hover:bg-white/[0.06] rounded-lg transition-all"
                  >
                    Login
                  </button>
                  <Button
