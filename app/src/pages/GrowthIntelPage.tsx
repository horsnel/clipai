          ...
          {TOOLS.map(t => (
            <button key={t.key} onClick={() => { if (!t.locked) setActiveTool(t.key); else onNavigate('pricing'); }}
              className={`p-4 rounded-xl border text-left transition-all relative ${
                activeTool === t.key
                  ? 'border-clip-cyan bg-clip-cyan/10 shadow-[0_0_16px_rgba(0, 255, 255,0.15)]'
                  : t.locked
          ...
          <div className="space-y-6">
            ...
            {spyResult && (
              <div className="space-y-4">
                <div className="card-glass p-5 border-clip-cyan/30 bg-clip-cyan/5 shadow-[0_0_24px_rgba(0, 255, 255,0.08)]">
                    ...
                    {PLATFORMS.map(p => (
                      <button key={p} onClick={() => setTimingPlatform(p)}
                        className={`w-full px-3 py-2 rounded-lg text-sm text-left transition-all ${
                          timingPlatform === p ? 'bg-clip-cyan text-black font-bold shadow-[0_0_10px_rgba(0, 255, 255,0.3)]' : 'bg-clip-surface text-clip-muted border border-white/[0.08] hover:text-clip-text'
                        }`}>{p}</button>
                    ))}
                  </div>
                </div>
