...
// ─── Audit Insights (extensive AI review of a channel) ───────────────────────
// Returned by POST /api/audit-insights. Generated by LLM from the cached audit
// data. Cached separately for 30min. Free for the user (no credit charge).
export interface AuditInsights {
  healthScore: number;            // 0-100
  healthLabel: 'Excellent' | 'Strong' | 'Moderate' | 'Needs Work' | 'Critical';
  ...
  bestPerformingVideos: Array<{
    ...
    comments: number;
    whyItWorked: string;
    replicationTip: string;
  }>;
  worstPerformingVideos: Array<{
    title: string;
    views: number;
    likes: number;
    comments: number;
